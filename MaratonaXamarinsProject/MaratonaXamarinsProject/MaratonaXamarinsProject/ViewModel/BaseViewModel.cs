...
namespace MaratonaXamarinsProject.ViewModel
{
    public class BaseViewModel : INotifyPropertyChanged
    {
        ...
        public async Task PushAsync<TViewModel>(params object[] args) where TViewModel : BaseViewModel
        {
            ...
            var viewTypeName = $"MaratonaXamarinsProject.{viewModelTypeName.Substring(0, viewModelTypeName.Length - viewModelWordLength)}Page";
            var viewType = Type.GetType(viewTypeName);

            var page = Activator.CreateInstance(viewType) as Page;

            if (viewModelType.GetTypeInfo().DeclaredConstructors.Any(c => c.GetParameters().Any(p => p.ParameterType == typeof(IMaratonaXamarinService))))
            {
                ...
                argsList.Insert(0, maratonaXamarinService);
